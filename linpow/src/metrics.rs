...
use crate::meminfo;
use crate::netif;
use crate::procstat::ProcStatState;
...
impl Sampler {
    pub fn new(interval_ms: u64) -> Self {
        ...
        // ── RAPL + per-process attribution ────────────────────────────────
        // Pinned to one thread so the per-process apportionment uses the same
        // energy_uj deltas the RAPL watts row publishes.
        {
            let m = shared.clone();
            let domains = rapl::enumerate();
            let mut states: Vec<DomainState> =
                (0..domains.len()).map(|_| DomainState::default()).collect();
            let mut procs = ProcStatState::new();
            ...
            spawn_periodic(&mut handles, &running, dt, move || {
                if domains.is_empty() {
                    // Even without RAPL, run the procstat tick so the Software
                    // section still populates with CPU tick deltas — energy
                    // attribution will be zero but the rank order still works.
                    let (top, total_w, total_e) = procs.sample(0, 0);
                    if let Ok(mut mg) = m.lock() {
                        mg.top_processes = top;
                        mg.all_procs_power_w = total_w;
                        mg.all_procs_energy_mj = total_e;
                    }
                    return;
                }
                let mut cores_w = 0.0;
                ...
                let mut cores_uj: u64 = 0;
                let mut uncore_uj: u64 = 0;
                for (d, s) in domains.iter().zip(states.iter_mut()) {
                    let w = s.tick(d);
                    match d.kind {
                        ...
                        DomainKind::Cores => {
                            cores_w += w;
                            cores_uj = s.last.energy_uj;
                        }
                        DomainKind::GpuUncore => {
                            gpu_w += w;
                            uncore_uj = s.last.energy_uj;
                        }
                ...
                let (top, total_w, total_e) = procs.sample(cores_uj, uncore_uj);
                if let Ok(mut mg) = m.lock() {
